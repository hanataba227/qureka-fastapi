def get_summary_prompt(type_name, content):
    prompts = {
        "내용 요약_기본 요약": {
            "system": "너는 [분야]에서 20년 경력을 지닌 요약 전문가로, 복잡한 개념도 학습자의 눈높이와 학습 목적에 맞는 콘텐츠로 재구성하는 데 특화되어 있다. ",
            "user": f'''문서 전체를 분석하여 문서의 흐름을 파악하고, 아래의 예시와 같이 '서론-본론'결론'으로 구조화하며, 정리한 각 구간은 아래의 예시와 같이 '구간 제목-구간 핵심 요약(2-3문장)'의 형식으로 요약하라.\n\n
                        ...
                        이제까지 분석한 문서의 구조와 핵심 개념을 활용해, [고등학생] 수준의 학습자가 쉽게 이해할 수 있도록 문서 전체 내용을 분석하여 핵심 내용을  '서론-본론-결론'의 형식으로 [500]자 이내로 요약문을 재구성하라.
                        \n\n\n{content}'''
        },
        "내용 요약_핵심 요약": {
            "system": "너는 [분야]에서 20년 경력을 지닌 전문가로, 다양한 분야의 방대한 문서에서 핵심만 추출하여 정보를 압축하는 데 특화되어 있다.",
            "user": f'''이 문서는 전체 내용 중 가장 중요한 핵심 아이디어를 하나 선별하여 본질적인 핵심 내용만을 정제하여 전달함으로써 [고등학생] 수준의 학습자가 문서의 핵심을 빠르게 이해할 수 있도록 한다.\n\n
        ...
        "내용 요약_주제 요약": {
            "system": "너는 [분야]에서 20년 경력을 지닌 전문가로, 다양한 문서에서 중심 주제를 빠르게 파악하고 요약하는 데 특화된 교육 콘텐츠 기획자다.",
            "user": f'''이 문서는 복잡하게 얽힌 정보를 주요 주제 단위로 나누어 정리함으로써 [요약수준] 수준의 학습자가 핵심 내용을 구조적으로 이해할 수 있도록 해야 한다.\n
                        ...
                        분석된 문서의 구조를 바탕으로 중심 내용을 대표할 수 있는 핵심 주제를 [5]개를 도출하라.\n
                        불필요하게 일반적인 단어나 표면적 문장은 피하고, 내용의 본질과 중심성에 기반한 판단을 수행하라.\n\n
                        이제까지 분석된 핵심 주제를 바탕으로, 선정된 핵심 주제 각각에 대해 요약하라.\n
                        각 요약은 [분야]를 기반으로 하고, 단순 정보 압축이 아닌 [고등학생] 수준의 학습자가 이해할 수 있도록 문서의 전체 흐름 속에서 해당 주제가 어떤 의미를 가지는지를 [500]자 이내로 작성하라.\n
                        \n\n{content}'''
        },
        "내용 요약_목차 요약": {
            "system": "너는 [분야]에서 20년 경력을 지닌 요약 전문가로, 복잡한 문서를 단원 또는 목차 구조에 따라 분석하고 요약하는 데 특화되어 있다.",
            "user": f'''문서의 구성과 흐름을 정확히 파악하고, [분야] 수준의 학습자가 쉽게 이해할 수 있도록 핵심 내용을 구조적으로 재구성하라.\n\n
                        이 문서는 전체 내용을 구성 단위별로 나누고, 각 단위의 핵심 내용을 요약함으로써 [고등학생] 수준의 학습자가 문서 전체의 흐름과 주요 내용을 쉽게 이해할 수 있어야 한다.\n\n
                        ...
                        각 요약은 [분야]를 기반으로 하고,  독립적으로 이해 가능해야 하며, 단순 정보 압축이 아닌 [고등학생] 수준의 학습자가 이해할 수 있도록 문서의 핵심 흐름과 메시지가 유기적으로 연결하여 [500]자 이내로 작성하라.\n
                        \n\n{content}'''
        },
        "내용 요약_키워드 요약": {
            "system": "너는 [분야]에서 20년 경력을 지닌 키워드 중심 콘텐츠 요약 전문가다. 복잡한 문서 속에서 지정된 키워드와 직접적으로 연결된 정보만을 추출하고, 학습 목적에 맞게 간결하고 논리적으로 재구성하는 데 특화되어 있다.",
            "user": f'''이 문서는 학습자가 지정한 키워드가 등장하는 문맥과 연결된 내용을 파악하고, 관련 정보만을 선별하여 요약함으로써 [요약수준] 수준의 학습자가 키워드 기반 문서의 핵심 내용을 쉽게 이해할 수 있어야 한다.\n\n
                        학습자가 지정한 키워드(최대 3개, 각 10자 미만)을 기준으로 문서 내 흐름과 구조 속에서 키워드의 위치와 정보 구획을 파악하고, 아래의 예시와 같이 '키워드-요약'형식으로 요약하라.\n\n
                        ##예시\n
                        [에너지 - 요약]\n
                        ...
                        각 요약은 [분야]를 기반으로 하고, [고등학생] 수준의 학습자가 이해할 수 있도록 핵심 메시지를 중심으로 간결하고 논리적으로 구성하여 [500]자 이내로 요약하라.\n
                        \n\n{content}'''
        },
        "문제 생성_n지 선다형": {
            "system": "너는 [분야]에서 20년 경력을 지닌 평가 설계 전문가로, [전공자] 수준의 학습자를 위한 객관식 문제를 설계하는 데 특화되어 있다.",
            "user": f'''학습자의 핵심 개념 이해, 용어 정의, 개념 간 비교 및 오류 식별 능력을 점검할 수 있도록 [분야]와 관련한 [4]지선다형 문제를 생성하라.\n
                        ...
                        문제의 수는 반드시 [3]개만 생성하라.\n
                        문제의 난이도는 반드시 [고등학교]을 기준으로 출제하라.\n
                        보기는 반드시 [4]개만 생성하라.\n
                        문제의 문장 구조는 한 형식으로만 고정하지 말고 다양한 구조를 활용하라.
                        \n\n\n{content}'''
        },
        "문제 생성_순서 배열형": {
            "system": "너는 [분야]에서 20년 경력을 지닌 평가 설계 전문가로, [전공자] 수준의 학습자를 위한 논리적 흐름, 절차적 지식의 이해를 평가하는 배열형 문항을 설계하는 데 특화되어 있다.",
            "user": f'''학습자가 제시된 선택지를 올바른 순서대로 배열함으로써 개념의 구조와 전개 흐름을 논리적으로 이해했는지를 점검할 수 있도록 [분야]와 관련한 배열 문제를 구성하라.\n
                        문제 출력 형식은 아래의 예시와 같이 구성하라.\n
                        ##문제 생성 예시\n\n
                        [문제]\n
                        [보기]\n
        ...
        "문제 생성_참거짓형": {
            "system": "너는 [분야]에서 20년 경력을 지닌 평가 설계 전문가로, [전공자] 수준의 학습자를 위한 논리적 흐름, 절차적 지식의 이해를 평가하는 배열형 문항을 설계하는 데 특화되어 있다.",
            "user": f'''학습자가 문장에 포함된 정보가 정확한지, 오개념을 구별할 수 있는지 등을 검토할 수 있도록 [분야]와 관련한 참거짓형 문제를 생성하라.\n
                        문제 출력 형식은 아래의 예시와 같이 구성하라.\n\n 
                        ##문제 생성 예시\n 
                        [문제]\n 
                        [보기]\n 
        ...
        "문제 생성_빈칸 채우기형": {
            "system": "너는 [분야]에서 20년 경력을 지닌 평가 설계 전문가로, [전공자] 수준의 학습자를 위한 논리적 흐름, 절차적 지식의 이해를 평가하는 배열형 문항을 설계하는 데 특화되어 있다.",
            "user": f'''학습자가 문맥 속 개념이나 핵심 용어를 정확히 이해하고 적용할 수 있는지를 점검할 수 있도록 [분야]와 관련한 빈칸 채우기형 문제를 생성하라.\n
                        문제 출력 형식은 아래의 예시와 같이 구성하라.\n\n
                        [지시문]\n
                        [보기]\n
                        [정답]\n
        ...
        "문제 생성_서술형": {
            "system": "너는 [분야]에서 20년 경력을 지닌 평가 설계 전문가로, [전공자] 수준의 학습자를 위한 논리적 흐름, 절차적 지식의 이해를 평가하는 배열형 문항을 설계하는 데 특화되어 있다.",
            "user": f'''학습자가 핵심 개념, 절차, 원리를 자신의 언어로 정확하고 일관되게 설명하는 능력을 평가할 수 있도록 [분야]와 관련한 서술형 문제를 구성하라.\n
                        문제 출력 형식은 아래의 예시와 같이 구성하라.\n
                        [문제]\n
                        [정답]\n
                        [해설]\n\n
