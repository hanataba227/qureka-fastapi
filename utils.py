    ...
    prompts = {
        ...
        "내용 요약_목차 요약": {
            ...
            "user": f"""문서를 구성 단위별로 나누고, 각 구간의 핵심 내용을 아래의 형식에 따라 요약하라.\n\n
                        ...
                        -단, '도입-제목'의 형식으로 출력하고, 제목은 반드시 해당 구간의 내용을 관통하는 핵심 주제(키워드)로 구성하라.\n 
        ...
        "내용 요약_키워드 요약": {
           "system": f"""너는 {field}에서 20년 경력을 지닌 전문가로, 복잡한 문서에서 지정된 키워드와 직접적으로 연결된 정보만을 추출하고, 학습 목적에 맞게 간결하고 논리적으로 재구성하는 데 특화되어 있다.\n
                          다음 규칙을 절대적으로 따라야 한다.\n\n
                          1. 문서 요약의 내용으로 반드시 {field_features(field)}의 관점을 유지하여 분석하라.\n
                          2. [주요 키워드] 단계에서 출력 내용은 반드시 사용자가 설정한 키워드인 {keywords}를 중심으로 구성하라. 절대 사용자가 설정하지 않은 키워드를 출력해서는 안 된다.(예를 들어, 사용자가 입력한 키워드 전부 문서와 관련이 없는 키워드인 경우, 아무것도 출력해서는 안 된다. 절대 임의의 키워드를 마음대로 출력하지 마라.)\n
            ...
            "user": f"""문서를 구성 단위별로 나누고, 각 구간의 핵심 내용을 아래의 형식에 따라 요약하라.\n\n
                        ...
                        -이때, 관련이 없는 키워드는 절대로 출력하지 마라. 사용자가 입력한 키워드도 절대 제공해서는 안 된다. 예를 들어, 문서와 관련이 없는 단어로 '뱀', '동전', 관련이 있는 단어로 '물' 입력된 경우, 키워드 출력 결과는 '물 - ~'만 제시해야 한다.\n
                        -만약, 사용자가 입력한 키워드 모두 문서와 관련이 없는 단어(예: '뱀', '동전')인 경우, '입력한 키워드 중 문서와 관련된 키워드가 없음'이라는 문구만을 출력하라. 그러나 이때 사용자가 입력한 키워드 중 문서와 관련이 있는 단어(예: '물')가 포함된 경우, 해당 키워드를 출력해야 한다..\n
