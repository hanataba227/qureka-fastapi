    ...
    prompts = {
        ...
        "내용 요약_키워드 요약": {
           "system": f"""너는 {field}에서 20년 경력을 지닌 전문가로, 복잡한 문서에서 지정된 키워드와 직접적으로 연결된 정보만을 추출하고, 학습 목적에 맞게 간결하고 논리적으로 재구성하는 데 특화되어 있다.\n
                          ...
                          2. [주요 키워드] 단계에서는 사용자가 설정한 {keywords} 중 문서와 직접적으로 관련된 키워드만을 선별하여 출력하고, 문서와 무관한 키워드는 절대 출력하지 마라.\n
                          3. [문서 분석] 단계에서는 도입-배경-전개-결론의 4단계 구조로 나누어 각 목차의 핵심 내용을 2-3문장으로 요약하라.\n
                          4. [통합 분석] 단계에서는 단순 요약이 아닌, 문서 구조와 키워드를 통합하여 '기술-적용-사회적 영향' 흐름에 따라 재해석하라.\n
                          5. [통합 분석] 각 구간은 반드시 {sentence_count}개의 완결된 문장으로 작성되어야 하며, 종결 표현(~다, ~이다 등)으로 끝나는 경우만 문장으로 인정한다. 쉼표로 연결된 복문은 하나의 문장으로 간주하므로, 반드시 문장을 구분하여 작성하라.\n
                          6. [통합 분석] 각 구간의 문장 수가 {sentence_count}개보다 적을 경우, GPT는 반드시 "이전 결과는 문장 수 기준을 충족하지 않아 다시 작성한다."는 문장을 출력한 후 전체를 다시 작성하라.\n
